...
const SCRIPT_MARKET_REPOSITORY_URL = "https://github.com/CloudRelay-Code/CodexForgeScriptMarket";
...
export function App() {
  ...
  const refreshPendingProviderImport = async (silent = true) => {
    ...
    if (result) {
      ...
      if (!silent && !isSuccessStatus(result.status)) showResultNotice(t("CodexForge 导入"), result, { silentSuccess: true });
    }
    return result;
  };
  ...
  const confirmPendingProviderImport = async () => {
    ...
    if (result) {
      ...
      setSettings(result);
      setSettingsForm(normalizeSettings(result.settings));
      showResultNotice(t("CodexForge 导入"), result);
      await refreshCcsProviders(true);
    }
  };
  ...
  const dismissPendingProviderImport = async () => {
    ...
    if (result) {
      setPendingProviderImport(null);
      showResultNotice(t("CodexForge 导入"), result, { silentSuccess: true });
    }
  };
  ...
  const restart = async () => {
    ...
    if (result) {
      showNotice(t("重启 CodexForge"), result.message, result.status);
      await refreshOverview(true);
    }
  };
  ...
  useEffect(() => {
    if (getLanguage() === "en") {
      void invoke("update_tray_labels", {
        ...
        quitLabel: "Quit",
        windowTitle: "CodexForge Manager",
      });
    }
  }, []);
        ...
        <div className="brand">
          ...
          <div className="brand-copy">
            <div className="brand-title-row">
              <div className="brand-title">CodexForge</div>
              {hasUpdate ? (
                <button
      ...
      <main className="workspace">
        <header className="topbar" key={`topbar-${route}`}>
          ...
          <div className="topbar-actions">
            ...
            <Button onClick={() => void actions.restart()} title={t("重启 CodexForge")} variant="outline">
    ...
    },
    {
      ...
      detail: result
        ? proxyVariables.length
          ? tf("检测到代理环境变量：{0}。请清理后重新启动 CodexForge。", [proxyVariableLabels.join(t("、"))])
...
function EnhanceScreen({
  ...
  const remoteMarketplaceSummary = remotePluginMarketplace?.marketplaceRoot
    ...
    : t("未发现本地缓存；点击按钮会从 CodexForge 内置快照释放并注册，无需官方账号预缓存。");
  return (
    <>
      <Panel>
        ...
        <CardContent>
          ...
          <div className="enhance-feature-groups">
            <FeatureGroup title={t("插件与模型")} detail={t("管理插件市场、模型列表和服务档位相关增强。")}>
              ...
              <div className="feature-action-row">
                <div>
                  ...
                  <small>{t("使用 CodexForge 内置快照补齐远端插件，API 模式也可显示和安装 Product Design 插件。")}</small>
            ...
            <FeatureGroup title={t("界面与启动")} detail={t("控制语言、启动速度和 Codex 原生界面调整。")}>
              ...
              <FeatureToggle title={t("原生菜单栏位置")} detail={t("把 CodexForge 菜单插入 Codex 顶部原生菜单栏。")} checked={form.codexAppNativeMenuPlacement} disabled={!masterEnabled} onChange={(value) => setEnhanceFlag("codexAppNativeMenuPlacement", value)} />
            ...
            <FeatureGroup title={t("远程项目")} detail={t("连接 Zed Remote 和 upstream worktree 辅助能力。")}>
              ...
              <FeatureToggle title={t("Zed 项目记录")} detail={t("维护 CodexForge 自己的远程项目最近列表。")} checked={form.zedRemoteProjectRegistryEnabled} disabled={!masterEnabled} onChange={(value) => setEnhanceFlag("zedRemoteProjectRegistryEnabled", value)} />
...
function ZedRemoteScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title={t("Zed 远程项目")} detail={tf("{0} 个 CodexForge 可识别项目，默认策略：{1}", [allProjects.length, zedStrategyLabel(form.zedRemoteOpenStrategy)])} />
        <CardContent>
          ...
          <div className="zed-remote-settings">
            ...
            <label className="switch-row compact">
              ...
              <span>
                ...
                <small>{t("保存到 CodexForge state，不改写 Zed settings。")}</small>
              </span>
            </label>
          </div>
          <Toolbar>
        ...
        <CardContent>
          ...
          <label className="switch-row">
            ...
            <span>
              ...
              <small>{t("开启后，通过 CodexForge 启动 Codex 前自动整理一次旧对话的归属标记。")}</small>
            </span>
          </label>
          <Toolbar>
...
function MaintenanceScreen({
  ...
  return (
    <>
      ...
      <Panel>
        ...
        <CardContent>
          <label className="check-row">
            ...
            <span>{t("卸载时移除 CodexForge 托管数据")}</span>
          </label>
          <Toolbar>
      ...
      <Panel>
        <CardHead title={t("自动接管")} detail={t("Watcher 用于保持 CodexForge 接管状态")} />
        <CardContent>
          <Toolbar>
      ...
      <Panel>
        ...
        <CardContent>
          ...
          <Toolbar>
            <Button onClick={() => void actions.launch()}>{t("启动 CodexForge")}</Button>
...
function AboutScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title={t("关于 CodexForge")} detail={t("本地 Codex 增强、管理工具和安装包维护")} />
        <CardContent>
          <div className="metric-list">
            <Metric label={t("CodexForge 版本")} value={overview?.current_version ?? update?.currentVersion ?? "-"} />
            <Metric label={t("Codex 版本")} value={overview?.codex_version ?? t("未检测到")} />
            <Metric label={t("项目地址")} value="github.com/CloudRelay-Code/CodexForge" />
          </div>
          <Toolbar>
            <Button onClick={() => void actions.openExternalUrl("https://github.com/CloudRelay-Code/CodexForge")} variant="secondary">
              <ExternalLink className="h-4 w-4" />
              {t("打开项目主页")}
            </Button>
            <Button onClick={() => void actions.openExternalUrl("https://github.com/CloudRelay-Code/CodexForge/issues")} variant="secondary">
              <ExternalLink className="h-4 w-4" />
              {t("反馈问题")}
            </Button>
          </Toolbar>
    ...
    <div className="relay-profile-editor">
      ...
      {showApiFields && profile.protocol === "chatCompletions" ? (
        <div className="hint-line relay-protocol-hint">
          ...
          <span>{t("此上游会通过本地 127.0.0.1:57321 转成 Responses API，需要从 CodexForge 启动 Codex。")}</span>
        </div>
      ) : null}
      <div className="hint-line relay-protocol-hint">
        <ShieldCheck className="h-4 w-4" />
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="modal-card provider-import-modal">
        <div className="modal-head">
          <div>
            <h2>{t("导入 CodexForge 供应商")}</h2>
            <p>{t("检测到来自网页的供应商配置导入请求，确认后会写入本机 CodexForge 管理工具。")}</p>
          </div>
...
function healthItems(overview: OverviewResult | null) {
  return [
    ...
    },
    {
      ...
      detail: overview?.silent_shortcut.path || t("缺少 CodexForge 静默启动快捷方式时可在安装维护页修复。"),
    },
    {
      title: t("管理工具入口"),
...
function applyRelayProfilePatchToFiles(
  ...
  if ("model" in patch) {
    // 模型后缀（如 [1M]）仅供 CodexForge 内部使用，写入 config.toml 前需剥离，
