...
#[test]
fn manager_queues_codexforge_provider_urls_for_confirmation_on_startup() {
    let main_rs = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/main.rs"))
        .expect("read manager main.rs");

    assert!(main_rs.contains("codexforge://"));
...
fn windows_binaries_request_administrator_privileges() {
    ...
    let windows_installer = manifest_dir
        ...
        .and_then(std::path::Path::parent)
        .unwrap()
        .join("scripts/installer/windows/CodexForge.nsi");
...
#[test]
fn windows_entrypoints_register_codexforge_url_protocol() {
    let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    ...
    assert!(windows_install.contains("Software\\Classes\\codexforge"));
...
fn macos_packager_hides_silent_launcher_but_not_manager() {
    ...
    assert!(script.contains("CodexForge-${VERSION}-macos-${ARCH}.dmg"));
    assert!(script.contains(
        "create_app \"CodexForge\" \"CodexForge\" \"$BINARY_DIR/codexforge\" \"com.cloudrelay.codexforge\" \"true\""
    ));
    assert!(script.contains(
        "create_app \"CodexForge 管理工具\" \"CodexForgeManager\" \"$BINARY_DIR/codexforge-manager\" \"com.cloudrelay.codexforge.manager\" \"false\""
    ));
}
