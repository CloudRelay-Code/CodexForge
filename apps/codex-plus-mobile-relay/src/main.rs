...
async fn main() -> anyhow::Result<()> {
    ...
    println!("CodexForge mobile relay listening on ws://{local_addr}");
    println!(
...
<head>
  ...
  <title>CodexForge Mobile Relay</title>
  <style>
...
<body>
<div class="app">
  <header class="topbar"><strong class="title">CodexForge</strong><span id="status" class="status">待连接</span></header>
  <main class="layout">
...
async function ensureAppServer() {
  ...
  await rpcRaw("initialize", { clientInfo: { name: "CodexForge Mobile Relay", version: "1.0.0" }, capabilities: { experimentalApi: true } });
}
