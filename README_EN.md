...
<p align="center">
  <img src="docs/images/codexforge.png" alt="CodexForge icon" width="160">
</p>
...
CodexForge is an external launcher and manager for the OpenAI Codex / ChatGPT desktop app. It uses the Chromium DevTools Protocol and a local helper for provider switching, protocol conversion, session management, and UI enhancements without modifying the official app's `app.asar` or installation files.

## Quick Start

Download the latest installer from [GitHub Releases](https://github.com/CloudRelay-Code/CodexForge/releases):

- Windows: `CodexForge-*-windows-x64-setup.exe`
- macOS Intel: `CodexForge-*-macos-x64.dmg`
- macOS Apple Silicon: `CodexForge-*-macos-arm64.dmg`

After installation, two entry points are available:

- `CodexForge`: silently starts the official desktop app with saved provider settings and enhancements.
- `CodexForge Manager`: manages providers, models, tools, sessions, enhancements, scripts, updates, and diagnostics.

For first-time setup, open the manager, verify the detected app path, configure a provider and optional enhancements, then launch through `CodexForge`. The Windows installer creates Desktop and Start Menu shortcuts. The macOS DMG installs `/Applications/CodexForge.app` and `/Applications/CodexForge 管理工具.app`.
...
Every UI enhancement is independently configurable. Disabling the global enhancement switch still leaves CodexForge available as a provider and launch manager.
...
Per-model windows accept values such as `1M`, `200K`, or plain integers. CodexForge generates a dedicated `model_catalog_json` for Codex.
...
Settings that depend on renderer injection generally require saving and restarting CodexForge.

## Updates and Packages

CodexForge publishes installers through GitHub Releases. Windows builds an NSIS installer, while macOS builds separate Intel x64 and Apple Silicon arm64 DMGs.
...
- CodexForge state and logs: `~/.codexforge/`
- Provider Sync backups: `~/.codex/backups_state/provider-sync`

## FAQ

### The CodexForge menu does not appear

Launch through the `CodexForge` entry instead of opening the official app directly. Check the detected app path, launch status, and diagnostic logs in the manager's Maintenance and About pages.
...
CodexForge updates the remote branch first, then creates the worktree as if you ran:

```bash
git worktree add -b <new-branch> <worktree-path> upstream/<base-branch>
```

The new worktree starts from the fresh remote tracking branch instead of the local HEAD used by the current session. If CodexForge cannot safely recognize the current Codex version's native worktree form, use the CodexForge menu entry and enter the repository path, branch name, worktree path, remote, and base branch manually.
...
scripts/installer/
  windows/CodexForge.nsi     Windows NSIS installer
  macos/package-dmg.sh          macOS DMG packager
```

## Feedback

If you run into problems or have feature suggestions, please open a [GitHub Issue](https://github.com/CloudRelay-Code/CodexForge/issues).

## License

This project is licensed under the [GNU Affero General Public License v3.0](LICENSE), SPDX identifier `AGPL-3.0-only`. The original copyright notice is preserved in [NOTICE](NOTICE). Modified versions that are distributed or offered to users over a network must provide the corresponding source code as required by AGPLv3.

The license covers CodexForge code only. It does not grant rights to OpenAI, ChatGPT, Codex trademarks, application assets, or other third-party content.

## Compatibility

CodexForge depends on the official desktop app's page structure, CDP behavior, and local data formats. Official app updates may require injection updates. Keep backups before changing provider configuration or local session data.
