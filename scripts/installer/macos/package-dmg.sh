...
DMG="$DIST/CodexForge-${VERSION}-macos-${ARCH}.dmg"
ICON_SOURCE="$ROOT/apps/codex-plus-manager/src-tauri/icons/icon.png"
ICON_NAME="codexforge.icns"
...
prepare_icon() {
  local iconset="$DIST/codexforge.iconset"
  rm -rf "$iconset"
  mkdir -p "$iconset"
...
sign_app "$STAGE/CodexForge.app"
sign_app "$STAGE/CodexForge 管理工具.app"

verify_app "$STAGE/CodexForge.app"
verify_app "$STAGE/CodexForge 管理工具.app"

ln -s /Applications "$STAGE/Applications"

hdiutil create -volname "CodexForge" -srcfolder "$STAGE" -ov -format UDZO "$DMG"
echo "$DMG"
