...
class BridgeConfig:
    ...
    state_dir: Path = field(
        default_factory=lambda: Path.home() / ".codexforge" / "wechat-claw"
    )
...
def load_config(path: Path) -> BridgeConfig:
    ...
    cfg = BridgeConfig(
        ...
        state_dir=Path(str(raw.get("state_dir") or "")) if raw.get("state_dir") else Path.home() / ".codexforge" / "wechat-claw",
        codex=CodexConfig(
