...
pub const SILENT_NAME: &str = "CodexForge";
pub const MANAGER_NAME: &str = "CodexForge 管理工具";
pub const SILENT_BINARY: &str = "codexforge";
pub const MANAGER_BINARY: &str = "codexforge-manager";
...
fn platform_install(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持安装 CodexForge 入口")
    }
}
...
fn platform_uninstall(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持卸载 CodexForge 入口")
    }
}
    ...
    if binary == SILENT_BINARY {
        ...
        return Some(
            macos
                ...
                .unwrap_or_else(|| macos.join("CodexForge")),
        );
    }
    if binary == MANAGER_BINARY {
        ...
        return Some(
            macos
                ...
                .unwrap_or_else(|| macos.join("CodexForgeManager")),
        );
    }
    None
}
