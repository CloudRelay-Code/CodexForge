...
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\CodexForge";
const LEGACY_UNINSTALL_SUBKEY: &str =
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall\CodexForge";
const URL_PROTOCOL_SUBKEY: &str = r"Software\Classes\codexforge";
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        ...
        uninstall_key: "CodexForge".to_string(),
        legacy_uninstall_key: "CodexForge".to_string(),
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    ...
    let install_location = Path::new(&plan.manager_path)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "CodexForge".to_string()),
        ("DisplayVersion", crate::version::VERSION.to_string()),
        ("Publisher", "CloudRelay-Code".to_string()),
...
fn register_url_protocol(manager_path: &str) -> anyhow::Result<()> {
    crate::windows_integration::set_current_user_string_value(
        URL_PROTOCOL_SUBKEY,
        "",
        "URL:CodexForge Import Protocol",
    )?;
    crate::windows_integration::set_current_user_string_value(
        URL_PROTOCOL_SUBKEY,
        "URL Protocol",
...
fn default_icon_path() -> PathBuf {
    std::env::current_exe()
        ...
        .map(|path| path.join("codexforge.ico"))
        .unwrap_or_else(|| PathBuf::from("codexforge.ico"))
}
