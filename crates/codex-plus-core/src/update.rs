...
pub const DEFAULT_REPOSITORY: &str = "CloudRelay-Code/CodexForge";
pub const DEFAULT_LATEST_JSON_URL: &str =
    "https://github.com/CloudRelay-Code/CodexForge/releases/latest/download/latest.json";
...
pub async fn fetch_latest_release(latest_json_url: &str) -> anyhow::Result<Release> {
    let client =
        crate::http_client::proxied_client(&format!("CodexForge/{}", crate::version::VERSION))?;
    let payload = client
        .get(latest_json_url)
...
fn is_macos_native_arch_asset(name: &str) -> bool {
    ...
    // Old filename shape: `CodexForge_1.0.9_x64.dmg`
...
fn is_macos_installer_asset(name: &str) -> bool {
    ...
    name.contains("codexforge") && name.ends_with(".dmg")
}
