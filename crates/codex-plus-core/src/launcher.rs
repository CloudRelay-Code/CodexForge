        ...
        if settings.enhancements_enabled {
            ...
            if injection_ready {
                ...
                let degraded = launch_status(
                    "running_degraded",
                    "Codex launched; CodexForge enhancements are still waiting for the page bridge.",
                    debug_port,
                    helper_port,
                    &app_dir,
                );
...
fn apply_codexplusplus_window_icon_after_launch(process_id: u32) {
    let icon_resource_path =
        std::env::current_exe().unwrap_or_else(|_| PathBuf::from("codexforge.exe"));
    tokio::spawn(async move {
        for attempt in 1..=30 {
