...
fn watcher_install_plan_registers_rust_launcher_at_logon() {
    let plan = build_watcher_install_plan("C:/Tools/codexforge.exe".into(), 9333);

    assert_eq!(plan.run_value_name, "CodexForgeWatcher");
    assert_eq!(
        plan.run_value,
        "\"C:/Tools/codexforge.exe\" --debug-port 9333"
    );
    assert_eq!(plan.shortcut_name, "CodexForgeWatcher.lnk");
    assert_eq!(plan.shortcut_target, "C:/Tools/codexforge.exe");
    assert_eq!(plan.shortcut_arguments, "--debug-port 9333");
}

#[test]
fn spawn_launcher_command_points_to_silent_binary_only() {
    let command = build_spawn_launcher_command("C:/Tools/codexforge.exe", 9444);

    assert_eq!(command[0], "C:/Tools/codexforge.exe");
...
fn launcher_process_filter_protects_current_process_ancestry() {
    let processes = [
        (10, 0, "codexforge.exe"),
        (20, 10, "codexforge.exe"),
        (30, 20, "codexforge.exe"),
        (40, 10, "codexforge.exe"),
        (50, 10, "codexforge-manager.exe"),
    ];
...
fn find_codex_processes_ignores_unrelated_processes() {
    let processes = [
        ...
        WindowsProcessInfo {
            process_id: 20,
            parent_process_id: 0,
            exe_file: "codexforge.exe".to_string(),
            executable_path: Some(std::path::PathBuf::from(
                r"D:\Programs\CodexForge\codexforge.exe",
            )),
        },
    ];
