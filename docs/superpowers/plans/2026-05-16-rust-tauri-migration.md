...
**Architecture:** Add a Rust workspace beside the current Python package, port behavior in verifiable slices, then switch the installed entry points to Rust and remove Python only after parity is proven. The only user-facing entry points are `CodexForge` silent launch and `CodexForge 管理工具`; there is no separate CLI.
...
- Create `apps/codex-plus-launcher/`: no-window silent launcher binary used by the `CodexForge` shortcut.
- Create `apps/codex-plus-manager/`: Tauri management console used by `CodexForge 管理工具`.
...
edition = "2024"
license = "MIT"
repository = "https://github.com/CloudRelay-Code/CodexForge"
...
async fn main() -> Result<()> {
    println!("CodexForge launcher {}", codex_plus_core::version::VERSION);
    Ok(())
}
```
...
pub fn run() {
    tauri::Builder::default()
        ...
        .expect("failed to run CodexForge manager");
}
```
...
- Copy assets: `assets/images/codexforge.ico`, `assets/images/codexforge.png`, sponsor images
...
Generate two shortcuts and one uninstall registry entry. The silent shortcut points to `codexforge.exe`; the management shortcut points to `codexforge-manager.exe`.
...
```text
/Applications/CodexForge.app
/Applications/CodexForge 管理工具.app
```
...
```text
双击 CodexForge：静默启动增强版 Codex。
双击 CodexForge 管理工具：安装、卸载、更新、设置、日志和诊断。
```
...
- `CodexForge` shortcut starts Codex without opening management UI.
- Codex App shows the CodexForge injected menu.
- Delete/undo/export/move/settings routes work from the injected UI.
- `CodexForge 管理工具` opens and shows Overview status.
