...
**Goal:** 为 CodexForge 供应商配置添加 Codex-only 的 MCP、skills、plugins 上下文管理，并支持供应商级上下文勾选、上下文大小和压缩上下文大小。
...
fn apply_relay_files_with_context_selection_writes_only_selected_context() {
    ...
    codex_plus_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "CodexForge"
[model_providers.CodexForge]
name = "CodexForge"
wire_api = "responses"
...
```powershell
Get-Process | Where-Object { $_.ProcessName -in @('codexforge','codexforge-manager') } | Select-Object ProcessName,Id,Path
```

Expected: 无输出。如果有输出，运行：

```powershell
Get-Process | Where-Object { $_.ProcessName -in @('codexforge','codexforge-manager') } | Stop-Process -Force
```
...
```text
Built application at: E:\Desktop\CodexForge\target\release\codexforge-manager.exe
```
