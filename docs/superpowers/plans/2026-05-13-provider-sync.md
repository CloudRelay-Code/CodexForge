...
**Goal:** Add an optional default-off CodexForge provider sync that runs before Codex launches and keeps historical conversations visible after `model_provider` changes.
...
- Create `codex_session_delete/settings_store.py`: backend-readable CodexForge settings store with defaults and atomic writes.
...
- Modify `codex_session_delete/inject/renderer-inject.js`: add backend-backed Provider sync toggle to CodexForge menu.
...
def test_provider_sync_prunes_backups_to_five(tmp_path):
    ...
    for index in range(6):
        ...
        (backup / "metadata.json").write_text(json.dumps({"managedBy": "CodexForge provider sync"}), encoding="utf-8")
...
def create_backup(home: Path, target_provider: str, changes: list[SessionChange]) -> Path:
    ...
    (backup_dir / "metadata.json").write_text(
        json.dumps({"managedBy": "CodexForge provider sync", "targetProvider": target_provider}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    return backup_dir
...
def prune_backups(home: Path, keep_count: int = BACKUP_KEEP_COUNT) -> None:
    ...
    for path in backup_root.iterdir():
        ...
        try:
            ...
            continue
        if metadata.get("managedBy") == "CodexForge provider sync":
            managed.append(path)
...
Add this test to `tests/test_renderer_script.py` near the CodexForge menu tests:
...
In the CodexForge menu click handler, after existing `[data-codex-plus-setting]` handling, add:
