...
**Goal:** Add upstream branch choices to Codex App's native start-new-chat branch dropdown so `New worktree` can start from `upstream/<base>` without a separate CodexForge dialog.

**Architecture:** Extend the existing upstream worktree backend defaults with structured upstream refs, then add a conservative renderer adapter that injects `Upstream` options into native branch menus and records the selected upstream ref for the existing native-create interception path. Keep the manual CodexForge dialog as fallback for now, but make the native dropdown path the primary UX.
...
```bash
hdiutil attach "dist/macos/CodexForge-1.1.5-local-upstream-dropdown-macos-$(uname -m).dmg" -nobrowse -readonly
ditto "/Volumes/CodexForge/CodexForge.app" "/Applications/CodexForge.app"
ditto "/Volumes/CodexForge/CodexForge 管理工具.app" "/Applications/CodexForge 管理工具.app"
codesign --force --deep --sign - "/Applications/CodexForge.app"
codesign --force --deep --sign - "/Applications/CodexForge 管理工具.app"
codesign --verify --deep --strict --verbose=2 "/Applications/CodexForge.app"
codesign --verify --deep --strict --verbose=2 "/Applications/CodexForge 管理工具.app"
hdiutil detach "/Volumes/CodexForge"
```
...
import shutil
root = Path('/Users/carson/.codex/worktrees/b4e7/CodexForge')
