...
- Existing installer scripts:
  - Windows: `scripts/installer/windows/CodexForge.nsi`
...
10. Stage Windows binaries into `dist/windows/app`:
    - `target/release/codexforge.exe`
    - `target/release/codexforge-manager.exe`
11. Build the NSIS installer using `scripts/installer/windows/CodexForge.nsi`.
12. Upload two artifacts:
    - `codexforge-windows-binaries`
    - `codexforge-windows-installer`
...
8. Upload the generated DMG as an artifact named for the architecture:
   - `codexforge-macos-x64-dmg`
   - `codexforge-macos-arm64-dmg`
...
2. Run a local repository build command that matches the existing project build path:
   - `npm --prefix /e/Desktop/CodexForge/apps/codex-plus-manager run build`
