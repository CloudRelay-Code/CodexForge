...
CodexForge will add an optional enhancement for Codex App worktree creation. The feature makes new worktrees start from a remote tracking branch such as `upstream/main`, instead of inheriting a stale local `HEAD` or unsynced branch state.
...
The implementation will first add a reliable Rust backend capability and a CodexForge menu entry. After that is tested, the renderer injection can enhance Codex App's native worktree creation UI when the current Codex version exposes a recognizable action point. If the native action point cannot be detected, CodexForge must show a clear fallback message and keep the menu entry usable.
...
- Provide a stable CodexForge menu workflow even when Codex App UI internals change.
...
CodexForge settings gains an `Upstream worktree` enhancement toggle. The toggle is enabled only when backend support is available.

When the user uses the CodexForge menu entry, CodexForge asks for:
...
On success, CodexForge shows the created worktree path and source ref. On failure, CodexForge shows a concise error and keeps the user's entered values so they can fix the issue.

When native Codex App worktree enhancement is active, the user can continue using Codex's normal worktree creation path. CodexForge intercepts or augments the action only when it can identify the repository path, new branch name, worktree path, and base branch. If any required value is ambiguous, CodexForge does not run Git. It shows a toast that says the native flow could not be safely enhanced and points the user to the CodexForge menu entry.
...
Add the menu workflow first. It should reuse existing CodexForge injected menu patterns instead of creating a visually separate system.
...
- If extraction is incomplete, do not prevent the native action unless CodexForge can show a clear fallback.
...
2. Creating `feature/test` at a new worktree path through CodexForge menu.
...
Phase 1 adds the Rust backend module, bridge routes, tests, and CodexForge menu entry.
