...
The new provider entity should preserve high-value compatibility with cc-switch while staying scoped to CodexForge needs.
...
`ProviderMeta` should start intentionally small and only cover fields that CodexForge will consume in this phase. It may include compatibility placeholders for future interop, but implementation should not depend on unused surface area.
