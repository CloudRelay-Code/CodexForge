...
Add a user-selectable sync target to the CodexForge session sync page, and align CodexForge provider discovery and sync behavior with `Dailin521/codex-provider-sync`.
...
3. More complete sync repair logic for global workspace state, matching the reference project where CodexForge currently differs.
...
5. Add saved/manual providers from CodexForge settings with source `Manual` once those settings fields exist.
...
Continue to preserve file modification time after rewriting, matching current CodexForge behavior and the reference project.
...
Align CodexForge global state repair with the reference `GlobalStateService.SyncWorkspaceRootsAsync()`.
...
CodexForge currently handles the first four groups partially; this design adds `open-in-target-preferences.perPath` and uses SQLite cwd stats to resolve stored paths in the same spirit as the reference project.
...
Expand `metadata.json` to include structured fields while preserving CodexForge's existing `managedBy` marker for pruning:
...
```json
{
  ...
  "managedBy": "CodexForge provider sync"
}
```
...
Improve CodexForge recovery semantics to match the reference project more closely.
