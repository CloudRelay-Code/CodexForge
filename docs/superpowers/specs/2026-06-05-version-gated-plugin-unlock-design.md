...
**Tech Stack:** Rust backend settings/launcher helper routes, CodexForge runtime JavaScript injection, static injection tests in `codex-plus-core`, React/Tauri manager settings UI only if a status hint is added.
...
Optional UI hint in the injected CodexForge menu:
