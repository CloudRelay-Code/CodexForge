...
> 日期：2026-06-24  
> 范围：CodexForge 后端核心（`codex-plus-core`）  
> 目标：让 CodexForge 应用 relay profile 时，根据 `model_list` 后缀语法自动生成 codex 原生 `model_catalog_json` 文件并写入 config.toml 指针，使 codex 客户端按模型识别真实上下文窗口。
